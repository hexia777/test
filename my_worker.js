// https://www.wbolt.com/customize-cloudflare-page-rule.html
...
const BYPASS_URL_PATTERNS = [/\/member\/.*/, /\/checkout\/.*/]
...
addEventListener('fetch', (event) => {
  ...
  const upstreamCache = request.headers.get('x-HTML-Edge-Cache')
  ...
  if (request && response) {
    ...
    if (cookieHeader && cookieHeader.length && bypassCookies.length) {
      ...
      for (const cookie of cookies) {
        ...
        for (const prefix of bypassCookies) {
