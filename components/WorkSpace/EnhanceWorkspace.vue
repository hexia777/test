    ...
    <three-columns-layouts class="layouts">
      ...
      <div class="enhance-main">
        ...
        <div class="main-center" :class="[isDialog && 'dialog-main-center']" ref="mainCenter">
            ...
            >
              <template v-if="work.afterImg">
                <p class="review-tag quicksand-blod">
                  ...
                  <span v-if="work.customWidth && work.customHeight">
                    {{ work.customWidth }} x {{ work.customHeight }} px
                  </span>
                  <span v-else-if="previewInfo && previewInfo.afterW">
          ...
          <!-- <more-features-dialog v-if="isDialog && work.afterImg && afterImageLoading" /> -->
      ...
      <template #r>
        <div v-show="isDownload" class="tab download-tab">
          ...
          <div class="tab-main">
            <div class="preview-recommend">
              ...
              <div class="enlarge-model" v-if="activeJob === WorkFlowType.sharpen && showRecommendModels">
                <sharpen-models
                  :active-sharpen-model="activeSharpenModel"
                  :process-loading="processLoading"
                  @handleModelSwitch="handleSharpenModelSwitch"
                />
              </div>
              <div
                class="enlarge-model"
                v-else-if="activeJob === WorkFlowType.enlarge && showRecommendModels"
              >
                <enlarge-models
                  :active-enlarge-model="activeEnlargeModel"
                  :process-loading="processLoading"
                  @handleModelSwitch="handleEnlargeModelSwitch"
                />
              </div>
            </div>
        ...
        <div v-show="!isDownload" class="tab setting-tab">
          ...
          <div class="tab-main scroll-bar">
            ...
            <div class="ai">
              <div
                v-for="(item, i) in workSettingData"
                class="ai-items icons"
                :class="[{ 'is-active': item.job === activeJob }]"
                :key="i"
                @click="handleJobSwitch(item)"
                v-track-label="$commonTrackLabel('job_switch_' + item.job)"
              >
                <div class="icons">
                  <i :class="['icon iconfont-single', item.icon]"></i>
                  <i :class="['icon_hover iconfont-single', `${item.icon}`]"></i>
                  <i :class="['icon_active iconfont-single', `${item.icon}`]"></i>
                </div>
                <p class="title">
                  {{ item.title }}
                </p>
            ...
            <div class="setting-dashboard">
              <p class="title" v-if="descript">
                {{ descript }}
              </p>
              <template v-if="activeJob === WorkFlowType.sharpen">
                <sharpen-job
                  :job="elkJob"
                  :config="currentWork[0]"
                  @changeConfig="changeConfig"
                  @changeSharpenActiveModel="changeSharpenActiveModel"
                  @changeActiveJob="changeActiveJob"
                />
              </template>
              <template v-else-if="activeJob === WorkFlowType.enlarge">
                <!-- <enlarge-works
                  :init-work-type="initWorkType"
                  :job="elkJob"
                  :image="currentImage"
                  :init-model="initEnlargeModel"
                  :init-scale="initScale"
                  @changeActiveJob="changeActiveJob"
                  @changeEnlargeActiveModel="changeEnlargeActiveModel"
                  @changeConfig="changeConfig"
              ...
              <template v-else>
                <denoise-job :config="currentWork[0]" @changeConfig="changeConfig" />
              </template>
            </div>
          </div>
          <div class="tab-footer">
...
<script>
  ...
  import DenoiseJob from '@/components/job/DenoiseJob'
  ...
  import EnlargeModels from '@/components/job/EnlargeModels.vue'
  import SharpenModels from '@/components/job/SharpenModels.vue'
  ...
  export default {
    components: {
      ...
      DenoiseJob,
      ImageCrop,
      ImageCropEdit: () => import('@/components/ImageCrop/components/ImageCropEdit'),
      // EnlargeWorks,
      // FreeDownloadA,
      RecommendAppDialog,
      MoreFeaturesDialog,
    ...
    data() {
      return {
        ...
        descript: this.$t('workspace.photo_descript'),
        processedLock: false,
        moveLock: false,
        rate: 1,
        maxw: 335,
        ...
        modalType: '',
        processedModels: {},
        activeEnlargeModel: WorkFlowType.enlarge3,
        initEnlargeModel: '',
        initScale: '',
        activeSharpenModel: WorkFlowType.sharpen3,
    ...
    computed: {
      showRecommendModels() {
        ...
        // const sourcePageUrls = ['image-sharpen', 'image-enlarger', 'image-enhancer']
        // return (
      ...
      recommendFeatures() {
        ...
        if (this.workType === 'enlarge') {
          return ['cartoonizer', 'sharpen', 'restorer']
        }
        if (this.workType === 'sharpen') {
          return ['upscaler', 'cartoonizer', 'restorer']
        }
        return []
      },
      ...
      filterJobs() {
        return [
          ...
          WorkFlowType.denoise,
          WorkFlowType.waifu,
          WorkFlowType.sharpen,
          WorkFlowType.real_esrgan2,
          WorkFlowType.real_esrgan,
          WorkFlowType.sharpen_cop,
        ]
      },
      currentImage() {
      ...
      workSettingData() {
        return [
          {
            title: this.$t('workspace.enlarge'),
            icon: 'icon-s-enlarge',
            descript: this.$t('workspace.enlarge_descript'),
            job: WorkFlowType.enlarge,
          },
          {
            title: this.$t('workspace.sharpen'),
            icon: 'icon-s-sharpen1',
            descript: this.$t('workspace.sharpen_descript'),
            job: WorkFlowType.sharpen,
          },
          {
            title: this.$t('workspace.denoise'),
            icon: 'icon-s-denoise',
            descript: this.$t('workspace.denoise_descript'),
      ...
      currentImage: {
        async handler(val, oldVal) {
          ...
          if (val && val.file && !val.uid) {
            ...
            this.waitTimes = 0
            this.uploading = true
            if (this.processStep === this.processStepMap.processed) {
    ...
    async mounted() {
      ...
      // if (['enlarge', 'waifu'].includes(this.workType)) {
      //   this.enhanceType = 'Enlarge'
      // }
      // if (['enhance', 'sharpen'].includes(this.workType)) {
      //   this.enhanceType = 'Sharpen'
      // }
      // if (['denoise'].includes(this.workType)) {
      //   this.enhanceType = 'Denoise'
      // }

      if (this.workType === 'enlarge') {
        this.activeJob = 'enlarge'
        this.elkJob = WorkFlowType.enlarge
        this.currentWork = [
          {
            name: WorkFlowType.enlarge,
            config: WorkFlowConfig(WorkFlowType.enlarge),
          },
        ]
      } else if (this.workType === 'waifu') {
        this.activeJob = 'enlarge'
        this.elkJob = WorkFlowType.waifu
        this.currentWork = [
          {
            name: WorkFlowType.real_esrgan,
            config: WorkFlowConfig(WorkFlowType.real_esrgan),
          },
        ]
      } else if (this.workType === 'enhance') {
        this.activeJob = 'sharpen'
        this.elkJob = WorkFlowType.sharpen
        this.currentWork = [
          {
            name: WorkFlowType.sharpen,
            config: WorkFlowConfig(WorkFlowType.onlyPortrait),
          },
        ]
      } else if (this.workType === 'denoise') {
        this.activeJob = 'denoise'
        this.elkJob = WorkFlowType.denoise
        this.currentWork = [
          {
            name: WorkFlowType.enlarge,
      ...
      } else if (this.workType === 'sharpen') {
        this.activeJob = 'sharpen'
        this.elkJob = WorkFlowType.sharpen
        this.currentWork = [
          {
            name: WorkFlowType.enlarge,
            config: WorkFlowConfig(WorkFlowType.onlySharpen),
          },
        ]
      } else {
        this.activeJob = 'sharpen'
      ...
      this.elkJob = this.workType === 'enhance' ? 'sharpen' : this.workType
      this.defaultFlows = []
      ...
      try {
        ...
        if (query.fileId) {
          ...
          if (query.scale) {
            ...
            this.initEnlargeModel = WorkFlowType.art_enlarge
            this.initScale = query.scale
          } else {
    ...
    methods: {
      ...
      changeEnlargeActiveModel(job) {
        this.activeEnlargeModel = job
      },
      changeSharpenActiveModel(job) {
        this.activeSharpenModel = job
      },
      handleJobSwitch(val) {
        this.activeJob = val.job
        this.descript = val.descript
        this.changeActiveJob(this.activeJob)
        if (val.job !== WorkFlowType.enlarge) {
          this.enlagerType = ''
        }
      },
      handleEnlargeModelSwitch(config, item) {
        this.activeEnlargeModel = item.job
        this.elkJob = item.elkJob || item.job

        if (this.processedModels[item.job]) {
          const preTransId = this.processedModels[item.job]
          const preItem = this.processeds.find((item) => preTransId === item.transId)
          if (preItem) {
            this.onApplyProcessedImage(preItem, 'recommend')
          }
          return
        }
        this.onStartRecommendProcess(config, { selectedModel: item.job })
      },
      handleSharpenModelSwitch(config, item) {
        this.activeSharpenModel = item.job
        this.elkJob = item.elkJob || item.job
      ...
      async onApplyProcessedImage(item, type) {
        ...
        if (item.workFlowType === WorkFlowType.sharpen) {
          this.activeSharpenModel = item.selectedModel
        } else if (item.workFlowType === WorkFlowType.enlarge) {
          this.activeEnlargeModel = item.selectedModel
        }
      ...
      initWorkSpace() {
        ...
        this.activeEnlargeModel = WorkFlowType.enlarge3
        this.activeSharpenModel = WorkFlowType.sharpen3
      },
      previewLoaded() {
      ...
      onStartRecommendProcess(config, extendParams = {}) {
        if (this.processLoading) {
          return
        }
        const process_continue_type = config.config[0].config.module_params.model_name

        this.$trace.traceCustomEvent({
          event: 'click',
          xargs_event_category: 'general_click',
          event_value: 'extra_process_continue',
          event_info: {
            process_continue_type,
          },
        })
        ...
        this.$trace.traceCustomEvent({
          ...
          event_value: 'process',
          event_info: {
            process_recommend_type: this.activeJob,
            recommend_name: 'continue',
          },
        })
        ...
        this.work = {
          ...
          // uid: this.preImageOriginUid,
          uid: this.work.uid || '',
          elkJob: config.job,
          elkName: config.name,
          isStandardConfig: true,
          selectedModel: extendParams.selectedModel,
          workFlowType: this.activeJob,
        }
        this.onStartProcess()
      },
      // 执行前检查图片是否上传完成
      ...
      setCurrentWork() {
        ...
        if (this.activeJob === WorkFlowType.sharpen) {
          selectedModel = this.activeSharpenModel
        } else if (this.activeJob === WorkFlowType.enlarge) {
          selectedModel = this.activeEnlargeModel
        }
        ...
        this.work = {
          ...
          ...this.currentWork,
          params: JSON.parse(
            JSON.stringify(this.currentWork.map((item) => ({ ...item, disabledPreprocessing: true })))
          ),
          // uid: this.preImageOriginUid,
          uid: this.work.uid || '',
          selectedModel,
          elkJob: this.elkJob,
          elkName: this.currentWork[0].elkName,
          workFlowType: this.activeJob,
        }
      },
      onBeforeStartProcess() {
        ...
        if (this.hasFileId) {
          ...
          const recommend_type = this.initScale ? 'enlarge' : pageUrlMap[this.pageUrl]
          const recommend_name = this.initScale ? 'enlarge' : 'recommend'
          ...
          this.$trace.traceCustomEvent({
            ...
            event_value: 'process',
            event_info: {
              process_recommend_type: recommend_type,
              recommend_name,
            },
          })
        }
        ...
        // if (
        //   this.activeJob === WorkFlowType.sharpen &&
        //   cutoutCityList.includes(localStorage.getItem(COUNTRY))
        // ) {
        //   this.work.elkJob = WorkFlowType.sharpen_cop
        // }
      ...
      async onPreview(work) {
        ...
        // 直接后端取数据不准，customize的直接取配置的值
        if (work.config && work.config.config) {
          work.config.config.forEach((configItem) => {
        ...
        this.previewInfo.afterW = customWidth || previewUrlData.picWidth
        this.previewInfo.afterH = customHeight || previewUrlData.picHeight
        if (!this.$getCookie('cloud_isFirstVisit') && !this.$getCookie('token')) {
          this.$trace.traceCustomEvent({
            event: 'online_work',
            xargs_event_category: 'process',
