...
export default {
  ...
  methods: {
    ...
    async onStartProcessV2(workName, options, extendsElkParams) {
      ...
      const workConfig = getWorksConfig({
        name: workName, // name需要和getWorksConfig的key一致
        // models: [{ name: '', options: {} }],
        options: { ...options, imageUid: this.work.uid }, // 用户输入参数
        out_params: this.work.out_params,
      })
      this.work.elk = this.work.elk
        ? {
            ...this.work.elk,
            job: workName,
            process_type: workName,
            model_name: workConfig.config[0].config.module_params.model_name,
          }
        : {
            job: workName,
            process_type: workName,
      ...
      try {
        ...
        if (this.formatJobTimelines) {
          jobTimelines = this.formatJobTimelines()
          transformParams.job_timelines = JSON.stringify(jobTimelines)
        }
        if (options.scale) {
          transformParams.scale = options.scale
        }
        if (maskUid) {
          transformParams.mask_uid = maskUid
        }
        this.$trace.traceCustomEvent({
          event: 'online_work',
          xargs_event_category: 'process',
        ...
        this.work.startTime = Date.now()
        const { data } = await Work.startWorkCommonApi(transformParams)
        //异常校验
        if (this.errorHandler(data)) {
          return
        }
        ...
        // todo work elk
        this.processELKV2(workConfig, params, extendsElkParams)
        //异常校验
        if (this.errorHandler(data)) {
          return
        }
        ...
        Work.pointCheck({ trans_ids: [work.transId] })
          .then((res) => {
            ...
            if (res.code === 200 || this.isWindowsFreeUser) {
              ...
              setTimeout(async () => {
                const { imageUrl, downloadFileName } = await getDownloadUrlFromS3(work.transId, issrc)
                getCredits()
